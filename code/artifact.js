...
window.artifact.setup = function() {
  ...
  $('#toolbox').append(' <a onclick="window.artifact.showArtifactList()" title="シャードポータルリストの表示">シャード</a>');
...
//          row += '<span class="fragments'+(data[type].target?' '+TEAM_TO_CSS[data[type].target]:'')+'">'+fragmentName+': #'+data[type].fragments.join(', #')+'</span> ';
    ...
    if (tableRows.length === 0) {
      html += '<tr><td colspan="2"><i>現在シャードポータルはありません</i></td></tr>';
    }
  ...
  html += "<hr />" +
        "<p>2015年夏、Nianticはシャードポータルについてのデータ形式を変更しました。 この変更について、私達はもはやわかりません:</p>" +
        "<ul><li>それぞれのチームがターゲットポータルをもちます</li>" +
        "<li>各シャードを決められたターゲットポータルへ運びます</li></ul>" +
        "<p>ポータルを選択し詳細やシャードナンバーを取得できます。しかし、ターゲットについての情報がありません。</p>";
