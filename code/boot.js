...
function createDefaultBaseMapLayers() {
  ...
  //Import IITC plugin: OpenStreetMap.org map tilesv0.1.1.20151111.74206
  // TODO モバイルで動かない
  //var osmOpt = {
  ...
  //var HumUrl='http://{s}.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png';
  //baseLayers['OpenStreetMap'] = L.tileLayer(osmUrl,{attribution:osmOpt,theme:'light_all'});
  //baseLayers['Humanitarian'] = L.tileLayer(HumUrl,{attribution:osmOpt,theme:'light_all'});
...
//    zoomAnimation: false,
  ...
  $.each(hiddenLayer, function(ind, layer){
    ...
    $('#portaldetails').html('<div class="layer_off_warning">' +
                            '<p><b>警告</b>: いくつかの標準レイヤーが非表示にされています。特定のポータル/リンク/フィールドが表示されていない可能性があります。</p>' +
...
window.setupPlayerStat = function() {
  ...
  var t = 'レベル:\t' + level + '\n' +
        ...
        'AP:\t' + digits(ap) + '\n' +
        (nextLvlAp > 0 ? 'レベルアップまで:\t' + lvlUpAp + ' AP' : 'Maximul level reached(!)') +
        '\n\n招待枠:\t'+PLAYER.available_invites +
        '\n\nヒント: プレイヤー情報はリロードした時に更新されます。 (F5)';
