...
window.getPosition = function() {
  ...
  if(getURLParam('latE6') && getURLParam('lngE6')) {
    ...
    lat = parseInt(getURLParam('latE6'))/1E6 || 0.0;
    lng = parseInt(getURLParam('lngE6'))/1E6 || 0.0;
    z = parseInt(getURLParam('z')) || 17;
    return {center: new L.LatLng(lat, lng), zoom: z};
  }

  if(getURLParam('ll')) {
    console.log("mappos: reading stock Intel URL params");
    lat = parseFloat(getURLParam('ll').split(",")[0]) || 0.0;
    lng = parseFloat(getURLParam('ll').split(",")[1]) || 0.0;
    z = parseInt(getURLParam('z')) || 17;
    return {center: new L.LatLng(lat, lng), zoom: z};
  }
  ...
  if(readCookie('ingress.intelmap.lat') && readCookie('ingress.intelmap.lng')) {
    ...
    lat = parseFloat(readCookie('ingress.intelmap.lat')) || 0.0;
    lng = parseFloat(readCookie('ingress.intelmap.lng')) || 0.0;
    z = parseInt(readCookie('ingress.intelmap.zoom')) || 17;
