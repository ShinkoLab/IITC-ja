...
window.setupDataTileParams = function() {
  ...
  if (niantic_params.ZOOM_TO_LEVEL && niantic_params.TILES_PER_EDGE) {
    ...
    dialog({
      title: 'IITC 警告',
      html: "<p>IITCは標準IntelからZOOM_TO_LEVELやTILES_PER_EDGEの設定値を見つけられませんでした。</p>" +
           "<p>代替の設定値を使用しますが、この問題はおそらくZOOM_TO_LEVELやTILES_PER_EDGEの設定値が変更されたことにより発生しています。" +
           " この設定値に相違がある場合マップを正常に表示できないことがあります。</p>",
    });
