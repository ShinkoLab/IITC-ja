...
//TODO? other status details...
  ...
  if (details) {
    ...
    if(levelDetails != 8) {
      ...
      levelDetails += " レベルのレゾネーターが次のポータルレベルに必要。";
    } else {
      levelDetails += "\n最大レベル";
    }
  }
  ...
  if (typeof android !== 'undefined' && android && android.intentPosLink) {
    ...
    var permaHtml = $('<div>').html( $('<a>').attr({href:permalinkUrl, title:'Create a URL link to this portal'}).text('ポータルへリンク') ).html();
    linkDetails.push ( '<aside>'+permaHtml+'</aside>' );

    // and a map link popup dialog
    var mapHtml = $('<div>').html( $('<a>').attr({onclick:posOnClick, title:'Link to alternative maps (Google, etc)'}).text('マップへリンク') ).html();
...
window.getPortalMiscDetails = function(guid,d) {
  ...
  if (d) {
    ...
    var title = maxOutgoing + ' 本のアウトリンクが可能\n' +
                links.outgoing + ' 本のｱｳﾄﾘﾝｸ\n' +
                links.incoming + ' 本のｲﾝﾘﾝｸ\n' +
                '(計 ' + (links.outgoing+links.incoming) + ' 本)';
    var linksText = ['リンク', 'ｱｳﾄ '+links.outgoing+' / '+'ｲﾝ '+links.incoming+' ', title];

    var player = d.owner ?
        '<span class="nickname">' + d.owner + '</span>' :
        '-';
    var playerText = ['オーナー', player];


    var fieldCount = getPortalFieldsCount(guid);

    var fieldsText = ['ﾌｨｰﾙﾄﾞ', fieldCount];
    ...
    if(attackValues.attack_frequency !== 0)
      randDetailsData.push([
        '<span title="attack frequency" class="text-overflow-ellipsis">攻撃増加</span>',
        '×'+attackValues.attack_frequency]);
    if(attackValues.hit_bonus !== 0)
      randDetailsData.push(['ﾋｯﾄﾎﾞｰﾅｽ', attackValues.hit_bonus+'%']);
    if(attackValues.force_amplifier !== 0)
      randDetailsData.push([
        '<span title="force amplifier" class="text-overflow-ellipsis">攻撃増幅</span>',
