// ==UserScript==
// @id             iitc-plugin-glympse@IITC-ja
// @name           IITC-ja plugin: Glympse Layer
// @category       Layer(IITC-ja)
// @version        0.1.0.@@DATETIMEVERSION@@-ja0.1
// @namespace      https://github.com/jonatkins/ingress-intel-total-conversion
// @updateURL      @@UPDATEURL@@
// @downloadURL    @@DOWNLOADURL@@
// @description    [@@BUILDNAME@@-@@BUILDDATE@@] Glympse Layer for IITC - IITCマップ上にGlympseユーザーを表示します
...
@@PLUGINSTART@@
	...
	window.plugin.glympseViewer.formatTime = function(timeInSeconds) {
		...
		timeString = ((days > 0) ? days + '日 ': '') + ((hours > 0) ? hours + '時間 ': '') + ((minutes > 0) ? minutes + '分 ': '') + ((seconds > 0) ? seconds + '秒': '');
		return timeString;
	}
	...
	window.plugin.glympseViewer.startHeartBeat = function () {
		...
		window.plugin.glympseViewer.memberUpdateTimer = setTimeout(window.plugin.glympseViewer.updateMembers, 5000);
		window.plugin.glympseViewer.eventUpdateTimer = setTimeout(window.plugin.glympseViewer.updateEvents, 15000);
		window.plugin.glympseViewer.status = '動作中';
	...
	window.plugin.glympseViewer.stopHeartBeat = function () {
		...
		window.plugin.glympseViewer.status = '停止';
		$('.glympse-status-input').val(window.plugin.glympseViewer.status);
	}
	...
	window.plugin.glympseViewer.drawMember = function(memberObj, memberProperties) {
		...
		<div class="glympse-popup-row"><strong>最終更新:</strong> <abbr title="' + new Date(memberObj.location[0][0]).toString() + '">' + (((memberProperties.end_time > Date.now()) && (typeof(memberProperties.completed) == 'undefined')) ? window.plugin.glympseViewer.formatTime(Math.round((Date.now()-memberObj.location[0][0])/1000)) + '前' : '期限切れ') + '</abbr></div>\
		<div class="glympse-popup-row"><strong>有効期限:</strong> <abbr title="' + new Date(memberProperties.end_time).toString() + '">' + (((memberProperties.end_time > Date.now()) && (typeof(memberProperties.completed) == 'undefined')) ? window.plugin.glympseViewer.formatTime(Math.round((memberProperties.end_time-Date.now())/1000)) + '後' : '期限切れ') + '</abbr></div>\
	...
	window.plugin.glympseViewer.glympseDialog = function() {
		...
		glympsePanel.append('<div class="info-text">タグ名を入力して表示</div>');
		...
		glympsePanel.append('<div class="glympse-status"><label>状態:</label> <input type="text" readonly="readonly" class="glympse-status-input"> <input type="button" class="stopHeartbeat" value="停止"></div>');
	...
	window.plugin.glympseViewer.openMembersDialog = function() {
		if (window.useAndroidPanes()) {
			...
			blah = dialog({
				...
				title: 'Glympse: ' + window.plugin.glympseViewer.viewTarget + ' 内のメンバー',
	...
	window.plugin.glympseViewer.updateMembersList = function(viewMembersPopup) {
		memberViewContent = '<div class="header-row"><div class="col-title">メンバー名</div><div class="col-title text-center">有効期限まで</div><div class="col-title text-right">最終更新</div></div>';
		$.each(window.plugin.glympseViewer.members, function(idx, memberObj) {
			var memberProperties = {};
			$.each(memberObj.properties, function(propIdx, propObj) {
				memberProperties[propObj.n] = propObj.v;
			...
			timeToExpiry = (memberProperties.end_time > Date.now()) ? window.plugin.glympseViewer.formatTime(Math.round((memberProperties.end_time-Date.now())/1000)) : '期限切れ';
