// ==UserScript==
// @id             iitc-plugin-portals-list@IITC-ja
// @name           IITC-ja plugin: show list of portals
// @category       Info(IITC-ja)
// @version        0.2.1.@@DATETIMEVERSION@@-ja0.1-fix0.1
...
window.plugin.portalslist.displayPL = function() {
  ...
  if (window.plugin.portalslist.getPortals()) {
    ...
    list = $('<table class="noPortals"><tr><td>ポータルが見つかりません!</td></tr></table>');
  }
  ...
  if(window.useAndroidPanes()) {
    ...
    dialog({
      ...
      title: 'ポータルリスト: ' + window.plugin.portalslist.listPortals.length + ' 個',
      id: 'portal-list',
      width: 700
    });
  }
...
//or switch to a stable sort so order of equal items doesn't change
  ...
  ["All", "Neutral", "Resistance", "Enlightened"].forEach(function(label, i) {
    ...
    cell.textContent = ["全て", "中立", "RES", "ENL"][i]+':';
    cell.title = 'この色のポータルのみ表示';
    $(cell).click(function() {
  ...
  container.append('<div class="disclaimer">表のタイトルをクリックすると並び替えできます。<br>' +
    '上部の <b>全て・中立・RES・ENL</b> 欄は、『この色のみ』『この色を隠す』事で表示ポータルを絞れます。</div>');
...
var setup =  function() {
  if(window.useAndroidPanes()) {
    android.addPane("plugin-portalslist", "ポータルリスト", "ic_action_paste");
    addHook("paneChanged", window.plugin.portalslist.onPaneChanged);
  } else {
    $('#toolbox').append('<a onclick="window.plugin.portalslist.displayPL()" title="現在表示されているポータルを一覧 [t]" accesskey="t">ポータルリスト</a>');
  }
